...
class CompanyJobController extends Controller
{
    ...
    {
        $user = Auth::user();
        $my_company = Company::with(['employer'])->where('employer_id', $user->id)->first();
        if ($my_company) {
            $company_job = CompanyJob::with(['catgory'])->where('company_id', $my_company->id)->paginate(10);
        } else {
            $company_job = collect();
        }
    ...
    public function store(StoreCompanyJobRequest $request)
    {
        ...
        return redirect()->route('admin.company_jobs.index')->with('success', 'Company Jobs created successfully.');
    }
